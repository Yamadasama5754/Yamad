...
class Kaguya extends EventEmitter {
  ...
  log([
    ...
    { message: `Error! An error occurred:\n${err.stack || JSON.stringify(err, null, 2)}`, color: "white" }
  ]);
  process.exit(1);
});
...
`));
  ...
  async performSecurityCheck(api) {
    // تعطيل فحص الأمان تماماً لتجنب Rate Limiting من Facebook
    // البوت سيعمل بحرية في أي مجموعة
  }
  ...
  async loadComponents() {
    ...
    try {
      ...
      // ✅ تسجيل الأحداث من eventFunctions فقط
      if (global.client.eventFunctions) {
        global.client.eventFunctions.forEach((fn, name) => {
          this.on(name, fn); // fn دالة، وليس كائن
        });
      }
    } catch (err) {
      failedCount++;
    ...
    (async () => {
      ...
      this.on("system:run", () => {
        login({ appState: JSON.parse(this.credentials) }, async (err, api) => {
          ...
          // فحص الأمان عند بدء البوت
          await this.performSecurityCheck(api);

          // تشغيل فحص الأمان كل 30 دقيقة بدلاً من مستمر
          ...
          // بدء نظام الأذكار المجدول
          try {
            const azkarEvent = global.client.events.get('greeting-azkar');
            if (azkarEvent && azkarEvent.scheduleAzkar) {
              await azkarEvent.scheduleAzkar(api);
            }
          } catch (error) {
            console.error('خطأ في بدء نظام الأذكار:', error);
          }
        });
      });
    })();
