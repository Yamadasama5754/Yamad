    ...
    switch (event.logMessageType) {
      ...
      case "log:subscribe": {
        ...
        if (event.logMessageData.addedParticipants.some((i) => i.userFbId == api.getCurrentUserID())) {
          ...
          // ✅ لا نطرد البوت من المجموعة، بل نقبل الإضافة بدل ذلك
          // هذا يحل مشكلة مجموعات لا تستقبل الرسائل
          try {
            const threadInfo = await api.getThreadInfo(event.threadID);
            const threadName = threadInfo.threadName || "Unknown";
            const membersCount = threadInfo.participantIDs?.length || 0;
            
            // إرسال رسالة للمطور في الخاص بكل إضافة
            const devMessage = [
              "═══════════════════════════",
              ...
              `👑 هل هو مطور: ${isDeveloper ? "✅ نعم" : "❌ لا (لكن تقبل البوت الإضافة)"}`,
              `⏰ الوقت: ${new Date().toLocaleString('ar-EG', { timeZone: 'Africa/Cairo' })}`,
              "═══════════════════════════"
            ].join("\n");
