  ...
  async execute({ event, api, args }) {
    try {
      ...
      try {
        ...
        api.setMessageReaction("❌", event.messageID, (err) => {}, true);
        return api.sendMessage("❌ | فشل الحصول على معلومات المستخدم", event.threadID, event.messageID);
      }
      ...
      try {
        const avatarResponse = await axios.get(
          `https://graph.facebook.com/${id}/picture?width=720&height=720&access_token=${this.fbToken}`,
